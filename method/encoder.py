...
class SelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, if_2d, in_degree, out_degree, dist_3d, dist_m, spe):
        ...
        # # --- 在方法最开始打印输入形状 ---
        # print(
        #     f"[SelfAttention INPUT Shapes] hidden_states: {hidden_states.shape}, attention_mask: {attention_mask.shape}, if_2d: {if_2d}")
        # if if_2d:
        #     print(f"  (2D) in_degree: {in_degree.shape}, out_degree: {out_degree.shape}, spe: {spe.shape}")
        # else:
        #     print(f"  (3D) dist_3d: {dist_3d.shape}, dist_m: {dist_m.shape}")
        ...
        try:
            result = attention_scores + attention_mask
        except RuntimeError as e:
            print("!!! Runtime Error during attention_scores + attention_mask !!!")
